...
public class AccountResource {
	...
	/*
	 * @PutMapping("/{accountNumber}") public ResponseEntity<Accounts>
	 * updateAccountBalance(@PathVariable int
	 * accountNumber,@RequestParam("currentBalance")Double currentBalance){
	 * Optional<Accounts> optional = service.getAccountById(accountNumber); Accounts
	 * accounts = optional.get(); if(accounts==null) { return new
	 * ResponseEntity<>(null, HttpStatus.NOT_FOUND); }
	 * currentBalance+=accounts.getCurrentBalance();
	 * accounts.setCurrentBalance(currentBalance); service.updateBalance(accounts);
